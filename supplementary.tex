...
This supplementary document provides the iterative refinement algorithm,
default hyperparameters, additional qualitative and cross-dataset figures,
the inference-step~/~refine-round ablation referenced in the main paper,
and a cross-dataset healthy false-positive probe.
...
Additional material not cited from the main text:
Table~S3 and Figures~S8--S11 (MAISI~/~NSCLC healthy-FP probe).

\section{Iterative Residual Mask Refinement}
\label{sec:supp:alg}
Algorithm~\ref{alg:refine} summarizes the residual-driven refine loop used
at inference (Eqs.~\eqref{eq:inject}--\eqref{eq:update} in the main paper).
...
Ours uses the same dataset-specific RF settings as Table~\ref{tab:cross}
...
(Table~\ref{tab:cross}). SAM-Med3D returns an empty mask on
\(\sim\)1\% of prompts (same rate as LIDC); those rows are present with
zero volume rather than missing.
...
Table~\ref{tab:cross} for true-nodule Dice).
