...
\(s=1/\mathrm{std}(\mathbf{x})\) estimated once from training data (MAISI-style)
and reused for training and inference; outputs are rescaled after sampling.
...
Given keep-mask \(\mathbf{m}\) and observation \(\mathbf{x}_0\), we sample
an \(N\)-step Euler discretization of the learned velocity ODE
(\(N{=}40\) by default), starting from
\(\mathbf{x}_t\sim\mathcal{N}(\mathbf{0},\mathbf{I})\) in scaled space.
Following RePaint~\cite{lugmayr2022repaint}, but without jump resampling,
known voxels are overwritten at each time \(t\) by a noised copy of the
...
If the resulting mask contains fewer than \(\eta{=}8\) voxels, we retain
...
\label{alg:refine}
\begin{algorithmic}[1]
\Require \(\mathbf{x}_0\), \(\mathbf{h}^{(0)}\), \(\mathcal{G}_\theta\),
\(R\), \(\tau\), \(\rho\), \(\eta\)
\Ensure \(\hat{\mathbf{y}}\)
\For{\(r = 0,\dots,R\)}
\State \(\hat{\mathbf{x}}^{(r)} \gets
  \mathcal{G}_\theta(\mathbf{x}_0,\,
  \mathbf{m}{=}1{-}\mathbf{h}^{(r)})\)
  \hfill\Comment{Eq.~\eqref{eq:inject}}
\If{\(r = R\)}
\State \textbf{break}
\EndIf
\State \(\mathbf{d}^{(r)} \gets
  |\mathbf{x}_0 - \hat{\mathbf{x}}^{(r)}|\)
  \hfill\Comment{Eq.~\eqref{eq:residual}}
\State \(\mathbf{s} \gets
  \mathrm{Dilate}(\mathbf{h}^{(r)};\,\rho)\)
\State \(\mathbf{h}' \gets
  \mathbb{1}[\mathbf{d}^{(r)} > \tau] \odot \mathbf{s}\)
  \hfill\Comment{Eq.~\eqref{eq:update}}
