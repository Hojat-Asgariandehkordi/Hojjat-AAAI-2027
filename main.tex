...
\item \textbf{Method.}
    ...
    masks during training (Figure~\ref{fig:method}).
...
VISTA3D~\citep{vista3d2024} (automatic + interactive CT foundation
model), MedSAM2~\citep{medsam2_2025}, and
nnInteractive~\citep{nninteractive2025}. These systems excel when an
object is present and well contrasted against the background.
However, they implement \emph{object} segmentation: the prompt
selects ``what to cut out,'' not ``whether anything abnormal is
there.'' They therefore lack an explicit healthy-tissue prior and
can emit confident masks on entirely normal parenchyma---precisely
the false-positive failure mode our healthy-FP probe targets.
Our setting reuses the same weak box prompts, but reads the answer
as residual evidence against a generative model of healthy lung.
...
Weak supervision from locations or RECIST diameters
\citep{deepem2018,zhou2023recist} reduces labeling cost yet still
trains a discriminative segmenter. Closest to our generative
view, \citet{rectflow2026} guide a 3D latent generative model to
suppress nodule signal under weak cues on LUNA16. We differ in
training an unconditional healthy-only prior, treating the prompt as
an inpainting hole with RePaint-style~\citep{lugmayr2022repaint}
known-region injection, and iteratively refining the hole from the
residual---without lesion masks at training time.

\paragraph{Generative Medical Anomaly Detection.}
Classical unsupervised AD learns a model of normality and scores
deviations, from GAN-based approaches such as
f-AnoGAN~\citep{fanogan2019} to diffusion-based reconstruction.
AnoDDPM~\citep{anoddpm2022} introduced partial diffusion with
simplex noise for brain MRI; AutoDDPM~\citep{autoddpm2023}
automates masking, stitching, and re-sampling for more robust
pseudo-healthy synthesis. Related lines include latent-space
partial diffusion~\citep{latent_ad2023}, counterfactual brain-tumor
generation~\citep{fontanella2023}, IterMask2~\citep{itermask2024},
Synomaly noise for ultrasound~\citep{synomaly2024}, and frequency
decomposition pipelines~\citep{fdp2026}. These methods typically
operate in 2D (or 2D-stacked) MRI/US, corrupt or mask regions
automatically, and do not consume the \emph{same interactive box
prompts} used by foundation segmenters. We bring the healthy-prior
idea into 3D lung CT CAD, where the candidate region is already
given and false positives on normal boxes are clinically costly.

\paragraph{Diffusion / Flow Inpainting and Adaptive Masks.}
RePaint~\citep{lugmayr2022repaint} established mask-guided
resampling for diffusion inpainting; industrial adaptive-mask
methods (AMI-Net~\citep{aminet2024}, MAD-paint~\citep{madpaint2025})
evolve corruption regions for anomaly localization outside medicine.
On the generative modeling side, denoising
diffusion~\citep{ho2020ddpm} and simulation-free flow
matching~/~rectified flow~\citep{lipman2023flowmatching,liu2023flow}
provide high-fidelity priors with straighter sampling paths.
We combine these ingredients for \emph{region-prompted anomaly
verification}: a 3D rectified-flow / diffusion prior of healthy
lung, box-defined holes, residual readout, and iterative
refinement---positioned as a complementary alternative to
interactive object segmenters when low false-positive rate matters.
...
and the online box-prompted inference loop, with example CT slices from a
LIDC patch.
...
\begin{figure*}[t]
  ...
  \textbf{Top:} a 3D rectified-flow model is trained on nodule-free patches to
  learn a healthy CT prior.
  \textbf{Middle:} at inference a box prompt defines an initial hole;
  mask-guided inpainting (using the prior) yields a healthy reconstruction;
  the residual \(|x-\hat{x}|\) is thresholded and the hole is refined for
  \(R\) rounds.
  \textbf{Bottom:} successive mask refinements on LIDC patch \#1370
  (red\,=\,GT, lime\,=\,prediction).
  Image panels show three consecutive axial slices to emphasize 3D \(64^{3}\) processing.}
  \label{fig:method}
\end{figure*}
...
(1658 cases), reaching a mean Dice of approximately \(0.73\), competitive with
SAM-Med2D (\(0.73\)) under matched box prompts.
...
For generalization we also evaluate matched box prompts on
MAISI synthetic nodule patches (\(N{=}109\)) and NSCLC-Radiomics
GTV volumes (\(N{=}85\)), using the same foundation baselines and
the same healthy-prior Ours family (dataset-specific RF inference
settings noted below).

\paragraph{Prompting.}
All methods receive the same Strategy-4 prompts: per-slice 2D
bounding boxes around the nodule / GTV mask with \(1\)-voxel
padding (the interactive-segmentation analogue of a weak clinical
cue). Ours converts these boxes into an inpainting hole; foundation
...
\emph{nodule-free} LIDC lung patches (\(N{=}3840\) prompts) and
measure how much mask each model still predicts. Because the ground
truth is empty, we report \emph{mean predicted volume}
(mm\(^3\)) rather than Dice-vs-empty (which conflates silent models
with perfect negatives).
...
residual refinement; unless noted, the default LIDC configuration
is \(40\) inference steps and \(R{=}2\) refine rounds.
...
(SAM-Med2D) and exceeds nnInteractive, SAM-Med3D, SegVol, and
VISTA3D, despite never training on lesion masks.
Figure~\ref{fig:results} summarizes the same comparison alongside
the healthy FP probe.

\begin{table}[t]
\centering
\caption{LIDC-IDRI positive test patches (\(N{=}1658\)), matched
Strategy-4 box prompts. Best Dice / IoU in \textbf{bold}.
SAM-Med3D uses the corrected full evaluation.}
\label{tab:main}
\begin{tabular}{lccc}
\toprule
Method & Prompt & Dice \(\uparrow\) & IoU \(\uparrow\) \\
\midrule
SAM-Med2D       & Box & \textbf{0.73} & \textbf{0.60} \\
\textbf{Ours (40-step, \(R{=}2\))}
                & Box & \textbf{0.73} & 0.59 \\
Ours (40-step)  & Box & 0.72 & 0.59 \\
nnInteractive   & Box & 0.71 & 0.58 \\
SAM-Med3D       & Box & 0.54 & 0.40 \\
SegVol          & Box & 0.37 & 0.25 \\
VISTA3D         & Box & 0.19 & 0.11 \\
\bottomrule
\end{tabular}
\end{table}
...
2D interactive segmentation and outperforms native-3D promptable
models that are weak under the same boxes.
Figure~\ref{fig:qual-lidc} visualizes matched box-prompt predictions
on LIDC cases with high Ours--GT agreement: the Ours contour nearly
coincides with GT, whereas SAM-Med2D often under-segments,
nnInteractive tends toward boxy over-segmentation, and SegVol /
VISTA3D leave noisy or incomplete masks under the same boxes.
The next experiment asks whether that competitiveness comes at the
cost of over-calling on normal lung.
...
\begin{figure*}[t]
  ...
  SAM-Med3D, SegVol, VISTA3D, and Ours (\(40\)-step, \(R{=}2\)).
  Ours (right) tightly follows the GT boundary on these cases.}
  \label{fig:qual-lidc}
\end{figure*}
...
Figure~\ref{fig:fp-tradeoff} places every method in the
true-nodule Dice vs.\ healthy FP volume plane.
...
Figure~\ref{fig:qual-fp} makes this concrete: under empty-GT random
boxes, SAM-Med2D / SAM-Med3D / SegVol often paint lime masks inside
the prompt, whereas Ours remains sparse
...
\begin{figure*}[t]
  ...
  \caption{Healthy false-positive probe: random boxes in nodule-free
  lung (yellow~=~prompt, lime~=~prediction; GT empty). Object-centric
  baselines often fill the box; Ours is more conservative
  (Table~\ref{tab:healthy_fp}).}
  \label{fig:qual-fp}
\end{figure*}
...
report mean Dice under the same matched-box protocol on MAISI and
NSCLC-Radiomics, with LIDC repeated for reference. Ours remains in the top tier on
all three sets (MAISI: \(60\)-step, \(R{=}3\); NSCLC: \(60\)-step,
\(R{=}1\)), tracking nnInteractive / SAM-Med2D while continuing to
beat SegVol and VISTA3D by a wide margin. Figure~\ref{fig:qual-cross}
shows the corresponding overlays on MAISI synthetic nodules and
matched NSCLC \(64^3\) tumor patches (same visual protocol as
LIDC/MAISI); Figure~\ref{fig:qual-nsclc-vol} shows the same four
NSCLC cases as full-volume slices. Ours again tracks irregular GT
boundaries closely, while the same foundation columns exhibit
under-segmentation, boxy inflation, or scattered false positives.
This suggests the healthy-prior residual is not an LIDC-only artifact.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/fig_qual_maisi.pdf}\\[0.4em]
  \includegraphics[width=0.95\textwidth]{figures/fig_qual_nsclc.pdf}
  \caption{Cross-dataset qualitative overlays (red~=~GT, lime~=~prediction).
  Top: MAISI synthetic nodules (same method columns as
  Figure~\ref{fig:qual-lidc}; Ours \(60\)-step/\(R{=}3\)).
  Bottom: NSCLC-Radiomics \(64^3\) tumor patches cropped from the
  processed volumes (same columns; Ours \(60\)-step, \(R{=}1\);
  cases LUNG1-024 / 145 / 098 / 173).
  On both sets, Ours hugs the GT contour more tightly than the
  interactive / foundation baselines.}
...
sufficient for near-peak Dice; additional steps yield diminishing
returns. Figure~\ref{fig:qual-ablation} shows the corresponding
overlays: coarse \(30\)-step residuals clean up by
\(40\)-step/\(R{=}2\), after which contours stay visually
stable---consistent with the flat Dice curve.
...
\midrule
30-step (refine on) & 0.72 & 0.58 \\
40-step (refine on) & 0.72 & 0.59 \\
\textbf{40-step, \(R{=}2\)} & \textbf{0.73} & \textbf{0.59} \\
60-step (refine on) & 0.72 & 0.58 \\
...
Across Tables~\ref{tab:main}--\ref{tab:healthy_fp} and
Figures~\ref{fig:results}--\ref{fig:fp-tradeoff}, the operating
point we recommend (\(40\)-step, \(R{=}2\)) is nearly tied with
SAM-Med2D on positive Dice while producing roughly half the
spurious predicted volume of SAM-Med2D---and about one quarter that
of nnInteractive---on healthy random boxes. Cross-dataset Dice
(Table~\ref{tab:cross}, Figures~\ref{fig:bench}--\ref{fig:bench-per})
shows the same competitive tier on MAISI and NSCLC. The ablation
(Table~\ref{tab:ablation}, Figures~\ref{fig:ablation} and
\ref{fig:qual-ablation}) confirms that extra sampling steps beyond
this point do not help. Qualitative model-output overlays
(Figures~\ref{fig:qual-lidc},~\ref{fig:qual-fp},~\ref{fig:qual-cross},
and~\ref{fig:qual-nsclc-vol})
make the quantitative story visible: under the same Strategy-4 boxes,
Ours recovers tight GT-aligned masks on LIDC/MAISI/NSCLC nodules,
stays sparse on healthy prompts relative to object-centric baselines,
and transfers without lesion-mask supervision. Together these
results support the problem reframing in Section~\ref{sec:intro}:
for region-prompted lung CAD, anomaly verification against a healthy
...
lesion masks at training time. Under matched prompts, the approach
is nearly tied with the strongest interactive baseline on LIDC true
nodules, remains in the top tier on MAISI and NSCLC, and is markedly
more conservative on healthy random boxes, where object-centric
segmenters often still fire. These results support generative
healthy inpainting as a principled alternative to interactive object
segmentation when low false-positive rate is critical in lung CT CAD.
