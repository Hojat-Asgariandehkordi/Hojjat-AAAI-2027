...
models that are weak under the same boxes. The next experiment asks
whether that competitiveness comes at the cost of over-calling on
normal lung.
...
\begin{figure}[t]
  ...
  (b)~predicted volume on healthy random boxes. Ours is nearly tied
  with SAM-Med2D on (a) while producing substantially less spurious
  mass on (b).}
  \label{fig:results}
\end{figure}
...
Table~\ref{tab:cross} and Figure~\ref{fig:bench} report mean Dice
under the same matched-box protocol on MAISI and NSCLC-Radiomics,
with LIDC repeated for reference. Ours remains in the top tier on
all three sets (MAISI: \(60\)-step, \(R{=}3\); NSCLC: \(60\)-step,
\(R{=}1\)), tracking nnInteractive / SAM-Med2D while continuing to
beat SegVol and VISTA3D by a wide margin. This suggests the
...
\midrule
SAM-Med2D     & \textbf{0.73} & 0.70 & 0.77 \\
...
\begin{figure*}[t]
  ...
  NSCLC for all methods. Hatched bars highlight Ours.}
  \label{fig:bench}
\end{figure*}
...
\textbf{40-step, \(R{=}2\)} & \textbf{0.73} & \textbf{0.60} \\
...
Figure~\ref{fig:fp-tradeoff}, the operating point we recommend
(\(40\)-step, \(R{=}2\)) is nearly tied with SAM-Med2D on positive
Dice while producing roughly half the spurious predicted volume of
SAM-Med2D---and about one quarter that of nnInteractive---on
healthy random boxes. Cross-dataset Dice
(Table~\ref{tab:cross}) shows the same competitive tier on MAISI
and NSCLC. Together these results support the problem reframing in
Section~\ref{sec:intro}: for region-prompted lung CAD, anomaly
verification against a healthy prior is a better inductive bias
than interactive object segmentation when false positives matter.
