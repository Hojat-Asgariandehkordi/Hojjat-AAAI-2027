...
\begin{figure*}[t]
  ...
  \textbf{Top:} training learns a 3D rectified-flow prior of healthy lung CT
  from nodule-free patches.
  \textbf{Bottom:} at inference a weak box prompt defines an initial hole;
  mask-guided inpainting produces a healthy reconstruction, the residual
  \(|x-\hat{x}|\) is thresholded to refine the hole, and the process is
  iterated for \(R\) rounds to yield the final segmentation
  (red\,=\,GT, lime\,=\,prediction).
  Inset shows successive mask refinements on LIDC patch \#1370.}
  \label{fig:method}
\end{figure*}
