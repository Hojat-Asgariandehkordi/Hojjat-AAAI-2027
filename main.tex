...
% Cross-references to the supplementary PDF (optional; S-macros below are authoritative)
\usepackage{xr}
...
\newcommand{\norm}[1]{\left\|#1\right\|}
% Stable pointers into supplementary.tex (S-numbering)
\newcommand{\suppTabHparams}{Table~S1}
\newcommand{\suppFigQualFP}{Figure~S1}
\newcommand{\suppFigBench}{Figure~S2}
\newcommand{\suppFigBenchPer}{Figure~S3}
\newcommand{\suppFigQualCross}{Figure~S4}
\newcommand{\suppFigQualNSCLC}{Figure~S5}
\newcommand{\suppTabAblation}{Table~S2}
\newcommand{\suppFigAblation}{Figure~S6}
\newcommand{\suppFigQualAblation}{Figure~S7}
\newcommand{\suppAlgRefine}{Algorithm~S1}
...
\(s_{\mathrm{std}}=1/\mathrm{std}(\mathbf{x})\) estimated once from the
LIDC training set (MAISI-style intensity scaling) and reused for training
and inference; outputs are rescaled after sampling. All ODEs below are
stated in this scaled space.
...
\suppTabHparams{} of the Supplementary Material.
...
\begin{proposition}[Known-region consistency under inject]
\label{prop:inject}
Fix an observation \(\mathbf{x}_0\) and keep-mask \(\mathbf{m}\).
Immediately after one step of~\eqref{eq:inject} at time \(t\)
(with \(s=t/T\)), every known voxel is reset to the noised observation,
while every hole voxel is left unchanged:
\begin{equation*}
x_{t,i}
=
\begin{cases}
...
\end{cases}
\end{equation*}
where \(x_{t,i}^{\mathrm{pre}}\) is the pre-inject state and
\(\boldsymbol{\varepsilon}'\sim\mathcal{N}(\mathbf{0},\mathbf{I})\).
In particular, the law of \(\mathbf{x}_t\big|_{\mathcal{K}}\) does not
depend on how the sampler evolved inside the hole \(\mathcal{H}\).
\end{proposition}
\begin{proof}
If \(i\in\mathcal{K}\), then \(m_i=1\) and~\eqref{eq:inject} sets
\begin{equation*}
x_{t,i}
\;\leftarrow\;
\tilde{x}_{t,i}(\mathbf{x}_0)
=
(1-s)\,x_{0,i}+s\,\varepsilon'_i.
\end{equation*}
If \(i\in\mathcal{H}\), then \(m_i=0\) and the same update leaves
\(x_{t,i}\) unchanged.
Thus \(\mathbf{x}_t\big|_{\mathcal{K}}\) depends only on
\((\mathbf{x}_0,\boldsymbol{\varepsilon}',t)\).
\end{proof}
...
(\suppAlgRefine{} in the Supplementary Material).
...
holes toward residual-supported lesion voxels; the full procedure is
listed as \suppAlgRefine{} in the Supplementary Material.
...
\suppFigQualFP{} in the Supplementary Material illustrates empty-GT
random boxes where SAM-Med2D / SAM-Med3D / SegVol often paint lime masks
inside the prompt, whereas Ours remains sparse.

\subsection{Generalization: MAISI and NSCLC}
Table~\ref{tab:cross} reports mean Dice under the same matched-box
protocol on MAISI~\citep{maisi2025} and
NSCLC-Radiomics~\citep{aerts2014nsclc}, with LIDC repeated for
reference; \suppFigBench{} and \suppFigBenchPer{} in the Supplementary
Material visualize the same comparison.
Ours remains in a competitive tier on all three sets
(MAISI: \(60\)-step, \(R{=}3\); NSCLC: \(60\)-step, \(R{=}1\)): within
rounding of nnInteractive on MAISI (both \(0.71\) at two decimals;
nnInteractive slightly higher before rounding) and within \(0.01\) Dice
of the best NSCLC entry (\(0.78\) vs.\ \(0.79\)).
\suppFigQualCross{} and \suppFigQualNSCLC{} in the Supplementary Material
show corresponding overlays on MAISI synthetic nodules, matched NSCLC
\(64^3\) tumor patches, and the same NSCLC cases as full-volume slices.
The qualitative pattern is consistent with the tables under matched
boxes, suggesting the healthy-prior residual is not an LIDC-only
artifact.
...
\suppTabAblation{}, \suppFigAblation{}, and \suppFigQualAblation{} in
the Supplementary Material vary inference steps and refine rounds \(R\)
...
Figure~\ref{fig:fp-tradeoff}, the operating point we
...
(Table~\ref{tab:cross}; \suppFigBench{}--\suppFigBenchPer{})
...
(\suppTabAblation{}, \suppFigAblation{}, \suppFigQualAblation{})
indicates that additional sampling steps beyond this point yield little
further Dice gain on LIDC. Qualitative overlays
(Figure~\ref{fig:qual-lidc}; \suppFigQualFP{}, \suppFigQualCross{}, and
\suppFigQualNSCLC{}) illustrate the same pattern under matched
...
We reframed region prompted unsupervised lung anomaly segmentation, unsupervised in the
...
\noindent
\emph{Disclaimer:}  OpenAI’s ChatGPT was used to assist with writing and editing. The authors reviewed and verified all content and accept full responsibility for the final manuscript.
% =============================================================
