...
verification} rather than generic interactive segmentation. We learn a
...
s=\tfrac{t}{T}.
\end{equation}
Differentiating~\eqref{eq:path} w.r.t.\ \(s\) gives a constant target velocity
\begin{equation}
\label{eq:velocity}
...
\end{equation}
independent of \(s\). Equivalently, the continuous-time generative ODE that
transports noise (\(s{=}1\)) to data (\(s{=}0\)) is
\begin{equation}
\label{eq:ode}
\frac{\mathrm{d}\mathbf{x}}{\mathrm{d}s}
=
v_\theta\!\bigl(\mathbf{x},\,sT\bigr),
\qquad
\mathbf{x}\big|_{s=1}\sim\mathcal{N}(\mathbf{0},\mathbf{I}).
\end{equation}
The network is trained by conditional flow matching~\cite{lipman2023flowmatching}
with an \(\ell_1\) regression loss on the straight-path velocity:
\begin{equation}
\label{eq:loss}
\mathcal{L}(\theta)
=
...
\label{prop:velocity}
For the interpolant~\eqref{eq:path}, the \(s\)-velocity is constantly
\(\mathbf{x}_0-\boldsymbol{\varepsilon}\) and the endpoint identities
\(\mathbf{x}_{t=0}=\mathbf{x}_0\), \(\mathbf{x}_{t=T}=\boldsymbol{\varepsilon}\)
hold.
\end{proposition}
\begin{proof}
Differentiate~\eqref{eq:path}:
\(\mathrm{d}\mathbf{x}_t/\mathrm{d}s
=\mathbf{x}_0-\boldsymbol{\varepsilon}\).
Substitute \(s{=}0\) and \(s{=}1\).
\end{proof}

\begin{proposition}[Exact \(x_0\) recovery]
\label{prop:x0}
If \(v_\theta(\mathbf{x}_t,t)=\mathbf{x}_0-\boldsymbol{\varepsilon}\)
along~\eqref{eq:path}, then
\begin{equation}
\label{eq:x0hat}
...
\end{equation}
recovers \(\mathbf{x}_0\) exactly for every \(s\in[0,1]\).
\end{proposition}
\begin{proof}
...
\label{cor:fm}
Any minimizer of~\eqref{eq:loss} with zero training risk yields a vector
field whose characteristics are the rectified straight paths of
Liu~et~al.~\cite{liu2023flow}; Euler integration of~\eqref{eq:ode} from
\(s{=}1\) to \(s{=}0\) therefore maps \(\boldsymbol{\varepsilon}\) to
\(\mathbf{x}_0\) in a single analytic step when \(v_\theta\) is exact, and
in \(N\) discrete steps otherwise.
\end{corollary}
...
\paragraph{Forward noising of known voxels.}
The rectified-flow forward map of an observation at time \(t\) is
\begin{equation}
\label{eq:forward}
\tilde{\mathbf{x}}_t(\mathbf{x}_0)
\;=\;
(1-s)\,\mathbf{x}_0
+
s\,\boldsymbol{\varepsilon}',
...
\end{equation}
i.e.\ the same interpolant~\eqref{eq:path} with a fresh noise draw.
Following RePaint~\cite{lugmayr2022repaint}, but without jump resampling,
we enforce known-region consistency at every reverse step by the
\emph{inject} operator
\begin{equation}
\label{eq:inject}
\mathcal{I}_{\mathbf{m},t}(\mathbf{x}_t;\mathbf{x}_0)
\;\coloneqq\;
...
\paragraph{Constrained Euler sampler.}
...
(\(N{=}40\) by default) and applies, for \(k=1,\dots,N-1\),
\begin{align}
\label{eq:euler-inject}
\mathbf{z}_{t_k}
&\;=\;
\mathcal{I}_{\mathbf{m},t_k}(\mathbf{x}_{t_k};\mathbf{x}_0),\\
\label{eq:euler-step}
\mathbf{x}_{t_{k+1}}
&\;=\;
\mathbf{z}_{t_k}
+
v_\theta(\mathbf{z}_{t_k},t_k)\,\Delta s_k,
\qquad
\Delta s_k=\tfrac{t_k-t_{k+1}}{T}.
\end{align}
After unscaling we obtain \(\hat{\mathbf{x}}\coloneqq\mathbf{x}_{t_N}\).
We write \(\hat{\mathbf{x}}=\mathcal{G}_\theta(\mathbf{x}_0,\mathbf{m})\)
for this mask-guided inpainting map.
...
\(\mathbf{x}_t\big|_{\mathcal{K}}\) is distributed exactly as the
RF forward process~\eqref{eq:forward} of \(\mathbf{x}_0\big|_{\mathcal{K}}\),
independent of the free evolution on \(\mathcal{H}\).
\end{proposition}
\begin{proof}
On indices \(i\in\mathcal{K}\), \(\mathcal{I}\) replaces the current state by
\(\tilde{x}_{t,i}(\mathbf{x}_0)\), which by~\eqref{eq:forward} equals
\((1-s)x_{0,i}+s\varepsilon'_i\).
Coordinates in \(\mathcal{H}\) are left unchanged by \(\mathcal{I}\) and
therefore do not enter the known-region law.
\end{proof}

\begin{proposition}[Exact reverse step along a path]
\label{prop:euler}
Suppose \(v_\theta=\mathbf{u}\) from~\eqref{eq:velocity} and
\(\mathbf{x}_{t_k}\) already lies on the interpolant~\eqref{eq:path} of
some pair \((\mathbf{x}_0^\star,\boldsymbol{\varepsilon})\).
Then one Euler update~\eqref{eq:euler-step} with step \(\Delta s_k\) lands
exactly on the same interpolant at time \(t_{k+1}\).
\end{proposition}
\begin{proof}
Along~\eqref{eq:path},
\(\mathbf{x}_{t_k}=(1-s_k)\mathbf{x}_0^\star+s_k\boldsymbol{\varepsilon}\)
with \(s_k=t_k/T\).
Then
\begin{align*}
...
\end{align*}
which is \(\mathbf{x}_{t_{k+1}}\) on the same path.
\end{proof}

\begin{corollary}[Boundary-consistent healthy fill]
\label{cor:inpaint}
Under Propositions~\ref{prop:inject}--\ref{prop:euler} with an exact
velocity field, the terminal sample satisfies
\(\hat{\mathbf{x}}\big|_{\mathcal{K}}=\mathbf{x}_0\big|_{\mathcal{K}}\)
almost surely (up to the final \(s{=}0\) inject), while
\(\hat{\mathbf{x}}\big|_{\mathcal{H}}\) is a healthy draw conditioned on
the observed context through the coupled ODE.
\end{corollary}

Intuitively, inject anchors the reverse process to the observation on
\(\mathcal{K}\); the learned healthy velocity then fills \(\mathcal{H}\)
so that \(\hat{\mathbf{x}}\) is a \emph{healthy counterfactual} of
...
(Algorithm~\ref{alg:refine}), casting mask update as a local residual test
against the healthy counterfactual.

\paragraph{Residual as an anomaly score.}
After inpainting with hole \(\mathbf{h}^{(r)}\) and keep-mask
\(\mathbf{m}^{(r)}=1-\mathbf{h}^{(r)}\), define the absolute residual in
...
\end{equation}
Large residuals mark voxels that the healthy prior cannot explain under the
current hole. The next hole is the thresholded residual restricted to a
local search band around the previous hole:
\begin{equation}
\label{eq:update}
\mathbf{h}^{(r+1)}
=
\1\!\bigl[\mathbf{d}^{(r)} > \tau\bigr]
\;\odot\;
\mathrm{Dilate}\!\bigl(\mathbf{h}^{(r)};\,\rho\bigr),
\end{equation}
with residual threshold \(\tau{=}0.05\) and 3D dilation radius \(\rho{=}2\).
If \(\lVert\mathbf{h}^{(r+1)}\rVert_0<\eta\) with \(\eta{=}8\), we retain
\(\mathbf{h}^{(r)}\) (stability safeguard).
We run \(R\) refinement rounds (\(R{=}2\) in our best configuration;
\(R{=}3\) in ablations), i.e.\ \(R{+}1\) inpaint passes in total.
No morphological open~/~close post-processing is applied by default.

\begin{assumption}[Healthy prior and covering hole]
\label{ass:healthy}
The trained field \(v_\theta\) is an exact healthy transport
(Corollary~\ref{cor:fm}).
The lesion support \(\mathrm{supp}(\mathbf{y})\) is contained in the current
hole, \(\mathrm{supp}(\mathbf{y})\subseteq\mathrm{supp}(\mathbf{h}^{(r)})\),
and intensities on
\(\mathrm{supp}(\mathbf{h}^{(r)})\setminus\mathrm{supp}(\mathbf{y})\)
are healthy (in-distribution for \(p_{\mathrm{healthy}}\)).
\end{assumption}

\begin{proposition}[Residual localization]
\label{prop:residual}
Under Assumption~\ref{ass:healthy} and exact boundary consistency
(Corollary~\ref{cor:inpaint}),
\begin{equation}
d^{(r)}_i
=
\begin{cases}
0, & i\notin\mathrm{supp}(\mathbf{h}^{(r)}),\\[2pt]
0, & i\in\mathrm{supp}(\mathbf{h}^{(r)})\setminus\mathrm{supp}(\mathbf{y}),\\[2pt]
\bigl|x_{0,i}-\hat{x}^{(r)}_i\bigr|>0
  & \text{almost surely if }\;
    x_{0,i}\;\text{is off the healthy manifold},
\end{cases}
\end{equation}
up to the stochasticity of~\eqref{eq:forward}.
Consequently, for any threshold
\(0<\tau<\inf_{i\in\mathrm{supp}(\mathbf{y})}d^{(r)}_i\),
the update~\eqref{eq:update} retains every lesion voxel that lies inside the
dilated search band and drops healthy voxels whose residual falls below
\(\tau\).
\end{proposition}
\begin{proof}
On \(\mathcal{K}^{(r)}\) (outside the hole), inject forces
\(\hat{x}^{(r)}_i=x_{0,i}\), hence \(d^{(r)}_i=0\).
Inside the hole but off the lesion, Assumption~\ref{ass:healthy} implies
the observation is a healthy draw; an exact healthy fill reproduces it in
expectation, so the residual vanishes in the noise-free limit.
On lesion voxels, \(\mathbf{x}_0\) lies off \(p_{\mathrm{healthy}}\) while
\(\hat{\mathbf{x}}^{(r)}\) is constrained to that manifold, so the
coordinates differ and \(d^{(r)}_i>0\).
Thresholding inside
\(\mathrm{Dilate}(\mathbf{h}^{(r)};\rho)\) therefore selects a subset of
anomalous coordinates without leaving the local search region.
\end{proof}

\begin{proposition}[Monotone localization under over-coverage]
\label{prop:mono}
Suppose \(\mathrm{supp}(\mathbf{y})\subseteq\mathrm{supp}(\mathbf{h}^{(r)})\)
and the search band does not introduce new false positives, i.e.\
\(\1[\mathbf{d}^{(r)}>\tau]\odot\mathrm{Dilate}(\mathbf{h}^{(r)};\rho)
\subseteq\mathrm{supp}(\mathbf{h}^{(r)})\)
whenever residuals outside \(\mathbf{h}^{(r)}\) are zero
(Proposition~\ref{prop:residual}).
Then
\(\mathrm{supp}(\mathbf{y})
\subseteq\mathrm{supp}(\mathbf{h}^{(r+1)})
\subseteq\mathrm{supp}(\mathbf{h}^{(r)})\):
each successful round weakly shrinks the hole while preserving lesion
coverage.
\end{proposition}
\begin{proof}
Lesion voxels have \(d^{(r)}_i>\tau\) and lie in \(\mathbf{h}^{(r)}\),
hence in its dilation, so they survive~\eqref{eq:update}.
Any voxel retained must lie in the dilation of \(\mathbf{h}^{(r)}\); under
the stated residual vanishing outside the hole and the inclusion
hypothesis, no coordinate outside \(\mathbf{h}^{(r)}\) is added, yielding
the nested supports.
\end{proof}
...
\STATE $\hat{\mathbf{x}}^{(r)} \gets \mathcal{G}_\theta\!\bigl(
  \mathbf{x}_0,\,\mathbf{m}{=}1{-}\mathbf{h}^{(r)}\bigr)$
  \COMMENT{Eqs.~\eqref{eq:inject}--\eqref{eq:euler-step}}
\IF{$r = R$}
\STATE \textbf{break}
\ENDIF
...
\STATE $\mathbf{h}' \gets \mathbb{1}[\mathbf{d}^{(r)} > \tau]
  \odot \mathbf{s}$
...
The loop therefore couples a geometric box prior with an appearance test
against the healthy flow: Propositions~\ref{prop:residual}--\ref{prop:mono}
formalize why over-complete initial boxes shrink toward the lesion under
residual thresholding, while Eq.~\eqref{eq:inject} keeps each iterate
anchored to observed healthy context.
