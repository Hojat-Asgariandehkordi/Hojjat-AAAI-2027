...
on representative LIDC cases: Ours tracks the GT contour closely,
while nnInteractive and SAM-Med3D often inflate the prompt box,
and SegVol / VISTA3D are noisy or incomplete under the same boxes.
...
boxes, interactive baselines often paint large lime masks inside the
prompt, whereas Ours remains sparse.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_qual_healthy_fp.pdf}
  \caption{Healthy false-positive probe: random boxes in nodule-free
  lung (yellow~=~prompt, lime~=~prediction; GT empty). Interactive
...
shows the corresponding overlays: on MAISI, the same foundation-model
columns as Figure~\ref{fig:qual-lidc} plus Ours; on NSCLC, volume-level
comparisons for the transferred checkpoint. This suggests the
healthy-prior residual is not an LIDC-only artifact.
...
\begin{figure*}[t]
  ...
  Top: MAISI (GT, SAM-Med2D, nnInteractive, SAM-Med3D, SegVol, VISTA3D,
  Ours \(60\)-step/\(R{=}3\)).
  Bottom: NSCLC-Radiomics with Ours (\(60\)-step, \(R{=}1\)).}
...
overlays: residual quality is visually stable across the reported
configs, consistent with the flat Dice curve.
...
make the quantitative story visible: Ours localizes nodules under the
same Strategy-4 boxes as the interactive baselines, while producing
less box-filling on healthy prompts and transferring to MAISI/NSCLC
without lesion-mask supervision. Together these
