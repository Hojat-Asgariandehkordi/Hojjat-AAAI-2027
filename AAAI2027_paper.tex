...
3D rectified-flow prior of \emph{healthy} lung CT from nodule-free
...
\(s=1/\mathrm{std}(\mathbf{x}_0)\) before the flow path is applied, and rescaled at the end of
sampling.
...
normalized space).
No lesion masks or ControlNet-style conditioning are used during training: the prior is a pure
...
MAISI synthetic nodule patches~\citep{maisi2025} (\(N{=}109\)) and
NSCLC-Radiomics GTV volumes~\citep{aerts2014nsclc} (\(N{=}85\)).
...
nodule / GTV mask with \(1\)-voxel padding (our ``Strategy-4'' box cue).
...
(SAM-Med2D; Dice \(0.73\) for both) and ahead of nnInteractive,
SAM-Med3D, SegVol, and VISTA3D, despite never training on lesion masks.
...
per-slice box prompts (Strategy-4). Best Dice / IoU in \textbf{bold}.}
\label{tab:main}
\begin{tabular}{lccc}
\toprule
...
the native-3D promptable models under the same boxes.
...
\begin{figure*}[t]
  ...
  Cases shown are high-agreement examples for Ours under the same prompts.}
  \label{fig:qual-lidc}
\end{figure*}
...
remain competitive on LIDC (Table~\ref{tab:main}), the recommended
Ours configuration (\(40\)-step, \(R{=}2\)) yields substantially
less spurious volume than SAM-Med2D~\citep{sam_med2d2023}, SegVol,
and especially nnInteractive~\citep{nninteractive2025}.
VISTA3D~\citep{vista3d2024} is nearly silent on healthy tissue, but
its LIDC Dice (\(0.19\)) shows that silence alone is not useful without
...
\(R{=}1\)): tied with nnInteractive on MAISI (\(0.71\)) and within
...
training time: solid, part-solid, and ground-glass findings are handled by
the same healthy-prior residual whenever they deviate from normal parenchyma.
