...
% Cross-references to the supplementary PDF
\usepackage{xr}
...
healthy counterfactual localizes the anomaly; an iterative refinement (RF)
...
We evaluate region-prompted anomaly detection against
...
MAISI synthetic nodule patches~\citep{maisi2025} (\(109\) cases) and
NSCLC-Radiomics~\citep{aerts2014nsclc} (\(85\) cases).
...
nodule segmentation mask with \(2\)-voxel padding.
...
\emph{nodule-free} LIDC lung patches (\(3840\) prompts) and report
...
\label{sec:exp:lidc}
Table~\ref{tab:main} reports matched-box true-nodule Dice~/~IoU together with healthy false-positive predicted volume (random boxes in nodule-free lung).
Ours is nearly tied with the strongest interactive baseline
(SAM-Med2D) and ahead of nnInteractive, SAM-Med3D, SegVol, and VISTA3D, despite
